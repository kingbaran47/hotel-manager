...
export const editRoom = async (req, res) => {
    ...
    try {
        const {rows} = pool.query("SELECT * FROM rooms WHERE id=$1", [id])
        if (rows === 0) return res.status(404).json(error, "Room does not exist.");
        const result = pool.query("UPDATE rooms SET size=$1, has_minibar=$2, is_available=$3 WHERE id=$4", [size, has_minibar, is_available, id])
