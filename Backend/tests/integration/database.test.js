...
// Helper: SQL-Datei lesen und Statements ausführen
...
beforeAll(async () => {
  // Tabelle + ENUM einmalig erstellen
  await runSqlFile(initFile);
});

beforeEach(async () => {
  // Daten zurücksetzen, ohne Tabelle/Typ zu droppen
  await pool.query('TRUNCATE TABLE rooms RESTART IDENTITY CASCADE');

  // Seed aus 002_seed_rooms.sql einfügen
...
describe('Rooms API Integration', () => {
  it('should return all rooms via service', async () => {
    ...
    expect(rooms.length).toBe(3); // entspricht der Seed-Datei
  });

  it('should have a suite room via service', async () => {
    const suiteRoom = (await pool.query("SELECT * FROM rooms WHERE size='suite'")).rows[0];
